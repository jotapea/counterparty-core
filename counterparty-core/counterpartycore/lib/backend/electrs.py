...
def indexer_check_version():
    ...
    # res = sc.call([client_request("server.version", "health_check", "1.4")])
    addrindexrs_version = INDEXER_THREAD.send({"method": "server.version", "params": ["health_check", "1.4"]})
    # addrindexrs_version = INDEXER_THREAD.send({"method": "server.version", "params": []})

    logger.info(f"addrindexrs_version aaa1: {addrindexrs_version}")
